...
echo "<span class=\"content-header\"><h3>" . $_GET['name'] . "</h3></span>";
...
if ($conn->connect_error) {
	...
	if (isset($_SESSION['is_admin']) && $_SESSION['is_admin']) {
		$adminConn = new mysqli($servername, $_SESSION['username'], $_SESSION['password'], $dbname);
		
		if (isset($_POST['newDbUsername'])) {
			$sql = "UPDATE Critic
					SET dbUsername = \"" . $_POST['dbUsername'] . "\"
		...
		echo "<table class=\"plain\">
				<form action=\"" . $selfURL . "\" method=\"post\">
					<tr><th>Database Username</th>
						<td><input type=\"text\" size=\"20\" name=\"dbUsername\" value=\"" . $dbUsername . "\" required></td>
						<td><button class=\"small-inline\" type=\"submit\" name=\"newDbUsername\">Update</button></td>
				</form>
			</table>";
	}
